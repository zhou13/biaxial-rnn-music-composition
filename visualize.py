...
def internalMatrixToImgArray(inmat):
    return np.array([[actToColor(m, a) for m, a in zip(row[:len(row) / 2], row[len(row) / 2:])]
                     for row in inmat])
...
def thoughtsToImageArray(thoughts):
    ...
    sequence = [
        spacer,
        probAndSuccessToImgArray(thoughts[4], thoughts[6], 0), spacer,
...
def drawPast(probs, succs):
    return np.array(
        [[pastColor(probs[time][note_idx], succs[time][note_idx]) for time in range(len(probs))]
...
def thoughtsAndPastToStackedArray(thoughts, probs, succs, len_past):
    ...
    def add_cur(ipt):
        return np.concatenate(
            (ipt, vert_spacer, probAndSuccessToImgArray(thoughts[-3], thoughts[-1], 0), vert_spacer,
             probAndSuccessToImgArray(thoughts[-3], thoughts[-1], 1)),
            axis=1)
    ...
    rows = [
        np.pad(row, ((0, 0), (maxlen - row.shape[1], 0), (0, 0)), mode='constant') for row in rows
    ]
