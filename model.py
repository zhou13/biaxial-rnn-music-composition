...
import theano_lstm
from theano_lstm import (LSTM, RNN, Embedding, Layer, MultiDropout, StackedCells,
                         create_optimization_updates, masked_loss)
...
class Model(object):
    ...
    def learned_config(self):
        return [
            self.time_model.params, self.pitch_model.params, [
                ...
                for mod in (self.time_model, self.pitch_model) for l in mod.layers if has_hidden(l)
            ]
        ]
    ...
    def learned_config(self, learned_list):
        ...
        for l, val in zip((l for mod in (self.time_model, self.pitch_model) for l in mod.layers
                           if has_hidden(l)), learned_list[2]):
    ...
    def setup_train(self):
        ...
        # We generate an output for each input, so it doesn't make sense to use the last output as
        # an input.  Note that we assume the sentinel start value is already present
        ...
        # apply dropout
        if self.dropout > 0:
            time_masks = theano_lstm.MultiDropout([(num_time_parallel, shape)
                                                   for shape in self.t_layer_sizes], self.dropout)
        else:
            time_masks = []

        time_outputs_info = [
            initial_state_with_taps(layer, num_time_parallel) for layer in self.time_model.layers
        ]
        time_result, _ = theano.scan(
            fn=step_time,
        ...
        # Now time_result is a list of matrix [layer](time, batch/note, hidden_states) for each
        # layer but we only care about the hidden state of the last layer.
        ...
        time_final = get_last_layer(time_result).reshape(
            (n_time, n_batch, n_note, n_hidden)).transpose((2, 1, 0, 3)).reshape(
                (n_note, n_batch * n_time, n_hidden))

        # note_choices_inputs represents the last chosen note. Starts with [0,0], doesn't include
        # last note.  In (note, batch/time, 2) format
        ...
        # Together, this and the output from the last LSTM goes to the new LSTM, but rotated, so
        # that the batches in one direction are the steps in the other, and vice versa.
        note_inputs = T.concatenate([time_final, note_choices_inputs], axis=2)
        num_timebatch = note_inputs.shape[1]

        # apply dropout
        if self.dropout > 0:
            pitch_masks = theano_lstm.MultiDropout([(num_timebatch, shape)
                                                    for shape in self.p_layer_sizes], self.dropout)
        else:
            pitch_masks = []

        note_outputs_info = [
            initial_state_with_taps(layer, num_timebatch) for layer in self.pitch_model.layers
        ]
        note_result, _ = theano.scan(
            fn=step_note,
        ...
        # Now note_result is a list of matrix [layer](note, batch/time, onOrArticProb) for each
        # layer but we only care about the hidden state of the last layer.  Transpose to be (batch,
        # time, note, onOrArticProb)
        note_final = get_last_layer(note_result).reshape((n_note, n_batch, n_time, 2)).transpose(
            1, 2, 0, 3)

        # The cost of the entire procedure is the negative log likelihood of the events all
        # happening.  For the purposes of training, if the ouputted probability is P, then the
        # likelihood of seeing a 1 is P, and the likelihood of seeing 0 is (1-P). So the likelihood
        # is (1-P)(1-x) + Px = 2Px - P - x + 1 Since they are all binary decisions, and are all
        # probabilities given all previous decisions, we can just multiply the likelihoods, or,
        # since we are logging them, add the logs.

        # Note that we mask out the articulations for those notes that aren't played, because it
        # doesn't matter whether or not those are articulated.  The padright is there because
        # self.output_mat[:,:,:,0] -> 3D matrix with (b,x,y), but we need 3d tensor with (b,x,y,1)
        # instead
        active_notes = T.shape_padright(self.output_mat[:, 1:, :, 0])
        mask = T.concatenate([T.ones_like(active_notes), active_notes], axis=3)

        loglikelihoods = mask * T.log(2 * note_final * self.output_mat[:, 1:] - note_final -
                                      self.output_mat[:, 1:] + 1 + self.epsilon)
    ...
    def setup_predict(self):
        ...
        def step_time(*states):
            ...
            notes_result, updates = theano.scan(
                fn=self._predict_step_note, sequences=[time_final], outputs_info=note_outputs_info)
        ...
        time_outputs_info = (
            [initial_state_with_taps(layer, num_notes) for layer in self.time_model.layers
                  ...
                  dict(initial=0, taps=[-1]), None])

        time_result, updates = theano.scan(
            fn=step_time, outputs_info=time_outputs_info, n_steps=self.steps_to_simulate)
    ...
    def setup_slow_walk(self):
        ...
        note_outputs_info = (
            [initial_state_with_taps(layer)
             for layer in self.pitch_model.layers] + [dict(initial=start_note_values, taps=[-1])])
        ...
        updates.update({self.walk_time: self.walk_time + 1, self.walk_input: next_input})
    ...
    def start_slow_walk(self, seed):
        ...
        for layer, hidden in zip((l for l in self.time_model.layers
                                  if has_hidden(l)), self.walk_hiddens):
            hidden.set_value(
                np.repeat(
                    np.reshape(layer.initial_hidden_state.get_value(), (1, -1)), num_notes, axis=0))
